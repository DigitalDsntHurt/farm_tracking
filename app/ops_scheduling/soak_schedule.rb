class SoakSchedule
	...
	##
	def self.get_orders_for_soak
		@orders = Order.where(cancelled_on: nil).where(standing_order: true) # gets all active standing orders
		@orders.select{|order| Crop.where(id: order.crop_id)[0].ideal_treatment_days > 0 } # filters standing orders on crops that require soak
	end		

	def self.get_soak_quantity(order)
		@crop = Crop.where(id: order.crop_id)[0]
	...
	##
	## ## methods that translate orders to a soak schedule
	##
	def self.orders_to_soak_instructions(orders)
	...
	def self.aggregate_crop_instructions(instructions)
		@return = []
	...
	def self.create_weekly_soak_schedule_array
  		aggregate_crop_instructions(orders_to_soak_instructions(get_orders_for_soak))
	end

	def self.instantiate_weekly_soak_schedule_db_dos
		create_weekly_soak_schedule_array.each{|hsh|
