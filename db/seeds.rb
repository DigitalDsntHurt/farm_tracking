...
=begin
##
## ## one-time set all default_medium values to "biostrate felt"
##

Crop.update_all(default_medium: "biostrate felt")
=end
...
=end
##
## ## one-time setup this week's sew todo in FarmOpsDos table (after this, they should be generated by a scheduler)
##

#SewSchedule.instantiate_weekly_sew_schedule_db_dos

@today = Date.today

@standard_return = SewSchedule.create_weekly_sew_schedule_array

@this_week_return = @standard_return.select{|hsh| hsh[:date].friday? }


@this_week_return.each{|hsh|
	FarmOpsDo.create(hsh)
}
=begin
##
## ## one-time setup this week's soak todo in FarmOpsDos table (after this, they should be generated by a scheduler)
##

@today = Date.today

@standard_return = SoakSchedule.create_weekly_soak_schedule_array

@this_week_return = []
@standard_return.each{|instruction_hsh|
	@hsh = {}
	instruction_hsh.each{|k,v|
		if k == :date and v != @today + 1
			@hsh[k] = v-7
		else
			@hsh[k] = v
		end
	}
	@this_week_return << @hsh
}
